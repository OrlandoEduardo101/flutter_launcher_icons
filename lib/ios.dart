...
class IosIconTemplate {
  IosIconTemplate({this.size = 0, this.name = ''});
...
void createIcons(Map<String, dynamic> config) {
  ...
  if (iosConfig is String) {
    ...
    for (IosIconTemplate template in iosIcons) {
      if (image != null) saveNewIcons(template, image, newIconName);
    }
  ...
  // update config file to use it
    ...
    for (IosIconTemplate template in iosIcons) {
      if (image != null) overwriteDefaultIcons(template, image);
    }
...
class ContentsInfoObject {
  ContentsInfoObject({this.version = 0, this.author = ''});
